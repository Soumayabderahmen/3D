...
import { Home, Building2, Trash2, Bath, ArrowRight, ArrowLeft, CheckCircle, Phone, MessageCircle, Clock, Star, Shield, Zap } from "lucide-react";
...
const volumes = ["Petit (< 10m³)", "Moyen (10-30m³)", "Grand (30-50m³)", "Très grand (50m³+)"];

const departments = ["Rhône (69)", "Ain (01)", "Isère (38)", "Loire (42)", "Saône-et-Loire (71)", "Drôme (26)", "Ardèche (07)", "Haute-Savoie (74)"];

const Devis = () => {
  const [step, setStep] = useState(1);
  const [submitted, setSubmitted] = useState(false);
  const [form, setForm] = useState({
    service: "", place: "", volume: "", departement: "", date: "", urgent: false,
    prenom: "", nom: "", tel: "", email: "", message: "",
  });
  ...
  if (submitted) {
    return (
      <Layout>
      <SEOHead {...getSEOForPath("/devis")} canonical="/devis" />
        <section className="py-32 bg-card">
          <div className="container text-center max-w-lg">
            ...
            <h1 className="font-display font-bold text-3xl text-foreground mb-4">Merci pour votre demande !</h1>
            <p className="text-muted-foreground mb-8">Notre équipe vous recontactera dans les 2 heures avec un devis personnalisé.</p>
                    ...
                    {step === 1 && (
                      <motion.div key="s1" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
                          {serviceTypes.map((s) => (
                            <button key={s.id} onClick={() => update("service", s.id)}
                              className={`p-4 rounded-xl border-2 text-center transition-all ${form.service === s.id ? "border-primary-accent bg-primary-accent/5" : "border-border hover:border-muted-foreground/30"}`}>
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 mb-8">
                          {placeTypes.map((p) => (
                            <button key={p.id} onClick={() => update("place", p.id)}
                              className={`p-4 rounded-xl border-2 text-center transition-all ${form.place === p.id ? "border-primary-accent bg-primary-accent/5" : "border-border hover:border-muted-foreground/30"}`}>
                              <p.icon className={`w-5 h-5 mx-auto mb-2 ${form.place === p.id ? "text-primary-accent" : "text-muted-foreground"}`} />
                              <span className="text-xs font-medium">{p.label}</span>
                            </button>
                          ))}
                        </div>
                        <button onClick={() => setStep(2)} disabled={!form.service || !form.place}
                    ...
                    {step === 2 && (
                      <motion.div key="s2" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
                        ...
                        <div className="grid grid-cols-2 gap-3 mb-6">
                          {volumes.map((v) => (
                            <button key={v} onClick={() => update("volume", v)}
                              className={`p-3 rounded-xl border-2 text-sm text-center transition-all ${form.volume === v ? "border-primary-accent bg-primary-accent/5 font-semibold" : "border-border hover:border-muted-foreground/30"}`}>
                              {v}
                            </button>
                          ))}
                        </div>
                        ...
                        <div className="flex gap-3">
                          <button onClick={() => setStep(1)} className="inline-flex items-center gap-2 px-6 py-3 rounded-full border border-border text-foreground font-semibold hover:bg-muted transition-colors">
                            <ArrowLeft className="w-4 h-4" /> Retour
                          </button>
                          <button onClick={() => setStep(3)} className="inline-flex items-center gap-2 px-6 py-3 rounded-full gradient-cta text-primary-foreground font-semibold">
                            Suivant <ArrowRight className="w-4 h-4" />
                          </button>
                        </div>
                      </motion.div>
                    ...
                    {step === 3 && (
                      <motion.div key="s3" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }}>
                        ...
                        <div className="mb-6">
                          ...
                          <textarea value={form.message} onChange={(e) => update("message", e.target.value)} rows={3} placeholder="Décrivez votre besoin..."
                            className="w-full px-4 py-2.5 rounded-xl border border-border bg-card text-foreground text-sm focus:ring-2 focus:ring-primary-accent focus:border-transparent outline-none resize-none" />
                        </div>
                        <div className="flex gap-3 mb-6">
                          <button onClick={() => setStep(2)} className="inline-flex items-center gap-2 px-6 py-3 rounded-full border border-border text-foreground font-semibold hover:bg-muted transition-colors">
                            <ArrowLeft className="w-4 h-4" /> Retour
                          </button>
                          <button onClick={handleSubmit}
                            className="flex-1 inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-full gradient-cta text-primary-foreground font-bold text-base shimmer">
            ...
            <div className="hidden lg:block">
              <div className="sticky top-24 space-y-4">
                <div className="bg-card rounded-2xl p-6 shadow-card border border-border">
                  ...
                  <a href="tel:0609991736" className="flex items-center gap-3 p-3 rounded-xl bg-primary-accent/5 text-primary-accent font-semibold mb-3 hover:bg-primary-accent/10 transition-colors">
