...
import { ArrowRight, Phone, CheckCircle, MapPin, Clock, Shield, Loader2 } from "lucide-react";
...
const ZoneServiceDetail = () => {
  ...
  if (!apiService || !subService || !zoneSlug) return <Navigate to="/services" replace />;
  ...
  const zoneName     = isLyon ? `Lyon ${arrNum}e` : zone.nom;
  ...
  const neighbors = isLyon
    ? Array.from({ length: 9 }, (_, i) => i + 1)
        .filter((n) => n !== arrNum)
        .slice(0, 6)
        .map((n) => ({ label: `Lyon ${n}e`, to: `/services/${apiService.slug}/lyon-${n}/${subServiceSlug}` }))
    : [];

  const faqs = [
    {
      q: `Quel est le délai pour un ${subService.title.toLowerCase()} à ${zoneName} ?`,
    ...
    {
      q: `Combien coûte un ${subService.title.toLowerCase()} dans le ${zoneNameFull} ?`,
      a: "Les tarifs varient selon le volume et l'accessibilité. Devis gratuit et sans engagement sur simple appel au 06 09 99 17 36.",
    },
    {
      q: `Intervenez-vous le week-end à ${zoneName} ?`,
      a: "Oui, nous intervenons 7j/7, y compris le week-end et les jours fériés.",
    },
    {
      q: `Quelles garanties pour votre service de ${subService.title.toLowerCase()} ?`,
  ...
  return (
    <Layout>
      <SEOHead
        ...
        canonical={`/services/${apiService.slug}/${zoneSlug}/${subServiceSlug}`}
        url={`/services/${apiService.slug}/${zoneSlug}/${subServiceSlug}`}
        jsonLd={[
          getServiceJsonLd(pageTitle, `${subService.title} professionnel à ${zoneName}`, `/services/${apiService.slug}/${zoneSlug}/${subServiceSlug}`),
          getBreadcrumbJsonLd([
            { name: "Accueil", url: "/" },
            { name: "Services", url: "/services" },
            { name: apiService.title, url: `/services/${apiService.slug}` },
            { name: zoneName, url: `/services/${apiService.slug}/${zoneSlug}` },
            { name: subService.title, url: `/services/${apiService.slug}/${zoneSlug}/${subServiceSlug}` },
          ]),
        ]}
      />
      ...
      <section className="gradient-primary py-16 relative noise-overlay">
        <div className="container relative z-10">
          <nav className="flex items-center gap-2 text-sm text-primary-foreground/50 mb-6 flex-wrap">
            ...
            <span>/</span>
            <Link to={`/services/${apiService.slug}/${zoneSlug}`} className="hover:text-primary-foreground/80 transition-colors">{zoneName}</Link>
            <span>/</span>
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            ...
            <span className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass text-sm text-primary-foreground/80">
              <MapPin className="w-4 h-4 text-secondary" /> {zoneName}
            </span>
          </div>
            ...
            <div className="grid lg:grid-cols-3 gap-12">
              ...
              <div className="lg:col-span-2 space-y-8">
                <div>
                  ...
                  <p className="text-muted-foreground leading-relaxed mb-4">{subService.long_desc}</p>
                    ...
                    <div className="p-6 rounded-xl bg-surface border border-border">
                      ...
                      <ul className="space-y-2">
                        {otherSubServices.map((s) => (
                          <li key={s.slug}>
                            <Link
                              to={`/services/${apiService.slug}/${zoneSlug}/${s.slug}`}
